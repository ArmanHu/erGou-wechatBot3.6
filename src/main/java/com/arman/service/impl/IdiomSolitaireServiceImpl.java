...
import com.arman.domain.UsedChengyu;
import com.arman.domain.WechatMsg;
import com.arman.idiom.GameStatus;
import com.arman.idiom.IdiomGame;
...
public class IdiomSolitaireServiceImpl implements IdiomSolitaireService {
	...
	@Override
	public void getNickNameByWxid(String roomid, String wxid) {
		if(!StringUtils.hasLength(roomid)){
			roomid = AjaxResult.CHATROOM;
		}
		if(!StringUtils.hasLength(wxid)){
	...
	@Override
	public void updateGameTime(String s) {
		...
		String nickName = MyFileUtil.getNickNameByWxid(wxid);
		IdiomGame.gameTime=2;
		WechatMsg wechatMsg = new WechatMsg();
        wechatMsg.setWxid(roomid);
		wechatMsg.setContent("@"+nickName+" 成语接龙次数已充值成功。");
		wechatBotService.sendTextMsg(wechatMsg);
